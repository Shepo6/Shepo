...
class SyFyResolver(UrlResolver):
    ...
    def get_media_url(self, host, media_id):
        ...
        try:
            url_snippit = re.search(r'''<meta property=["']og:image["'] content=["'].+?/image/(.+?)\d+x''', html).groups()[0]
            url_template = 'https://tvesyfy-vh.akamaihd.net/i/prod/video/%s,25,40,18,12,7,4,2,00.mp4.csmil/master.m3u8?__b__=1000&hdnea=st=%s~exp=%s'
